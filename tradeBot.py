...
def main(argv):
        ...
        try:
                #opts/options are flags and args is period retrieved from argv
                opts, args = getopt.getopt(argv,"hp:c:n:s:e:",["period=","currency=","points="])#hp: help feature that tells what flags application accepts
        ...
        for opt, arg in opts:
                if opt == '-h':
                        ...
                        if(int(arg) in [300,900,1800,7200,14400,86400]):#api set periods in seconds, must adhere too
                                period = arg
                        else:
        ...
        con = poloniex('keys go here','keys go here')
        ...
        while True:
                if(startTime and historicalData):
                        ...
                        currentValues = con.api_query("returnTicker")
                        lastPairPrice = currentValues[pair]["last"]
                        lastPairPrice = float(lastPairPrice)
